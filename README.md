...
- 提供属性延迟生效的功能，减少多次调用setData；通过延迟时间控制宏观上控制setData提交的频次和数量。
- 自动将Page实例的`data`中的属性和Component实例的`data`中的属性转换为同名的`getter`和`setter`（自动提交属性变动），简化使用方式。
- 提供增强模块管理，可以基于begonia扩展功能。并基于模块生命周期细粒度的控制创建小程序Page实例和Component实例的过程。
